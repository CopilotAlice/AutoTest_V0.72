...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.setWindowTitle("通用自动测试上位机_蔡_功能测试版_2411_V0.72")
        ...
        self.comboBox_binding_com_60s.setCurrentIndex(0)
        # 激光惯导#60所 自动装订构造
    ...
    def show_message_1s(self):
        ...
        self.long_time = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        ...
        self.lineEdit_debug_message_list[2].append('总接收:{}'.format(self.all_rec_hex))
            ...
            if len(all_data)>=decode_fram_leng*2:
                ...
                if frame.startswith(decode_rule_header) & next_frame.startswith(decode_rule_header): 
                    ...
                    if len(self.drop0data)>0:
                        last_not0data = [ last_not0data[i] if (int(receive_data_hz[i])==0) else receive_data_hz[i]  for i in range(len(receive_data_hz))]
